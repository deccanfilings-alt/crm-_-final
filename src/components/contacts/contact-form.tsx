...
export function ContactForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // If onBlur didn't fire (e.g. user pressed Enter or clicked Submit directly),
    // perform pre-flight duplicate check before attempting insert.
    if (!isEdit && accountId) {
      try {
        const existing = await findExistingContact(supabase, accountId, phone.trim());
        if (existing) {
          const exact = isExactMatch(existing, phone.trim());
          setDupMatch({ contact: existing, exact });
          if (exact) {
            toast.error('A contact with this phone number already exists');
            setSaving(false);
            return;
        ...
        console.warn('Duplicate pre-check failed, continuing to backstop:', checkErr);
      }
    }
