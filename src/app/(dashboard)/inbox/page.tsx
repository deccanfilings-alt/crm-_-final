      ...
      setConversations((prev) => {
        const existingIndex = prev.findIndex(
          (c) =>
            ...
            (c.contact_id &&
              fetched.contact_id &&
              c.contact_id === fetched.contact_id &&
              (c.channel || "whatsapp") === (fetched.channel || "whatsapp"))
        );
        if (existingIndex !== -1) {
          const updated = [...prev];
          updated[existingIndex] = {
            ...updated[existingIndex],
            ...fetched,
            contact: fetched.contact ?? updated[existingIndex].contact,
          };
          return updated;
        }
        return [fetched, ...prev];
          ...
          setConversations((prev) =>
            prev.map((c) =>
              c.id === newMsg.conversation_id
                ? {
                    ...c,
                    last_message_text: newMsg.is_internal
                      ? `[Internal Note] ${newMsg.content_text || `[${newMsg.content_type}]`}`
                      : (newMsg.content_text ?? ""),
                    last_message_at: newMsg.created_at,
                    unread_count:
                      activeConversation?.id === newMsg.conversation_id || newMsg.is_internal
                        ? (activeConversation?.id === newMsg.conversation_id ? 0 : c.unread_count)
                        : c.unread_count + 1,
                  }
                : c,
            ),
