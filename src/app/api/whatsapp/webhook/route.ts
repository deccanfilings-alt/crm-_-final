...
async function findOrCreateContact(
  ...
  if (existingContact) {
    // Only update name from WhatsApp profile if existing contact doesn't have a name yet
    // or if their name is currently just their raw phone number.
    // Never overwrite a custom or edited name saved by CRM users.
    const isNameEmptyOrPhone =
      !existingContact.name ||
      existingContact.name.trim() === '' ||
      existingContact.name === existingContact.phone ||
      existingContact.name === phone;

    if (name && isNameEmptyOrPhone) {
      await supabaseAdmin()
        .from('contacts')
...
async function findOrCreateConversation(
  ...
  // Order by last_message_at DESC, created_at DESC with limit 1 so even if
  // multiple rows ever exist, we reliably resolve the latest active thread.
  const { data: convs, error: findError } = await supabaseAdmin()
    .from('conversations')
    .select('*')
    .eq('account_id', accountId)
    .eq('contact_id', contactId)
  ...
  if (!findError && convs && convs.length > 0) {
    const existing = convs[0]
    // If the conversation was closed, automatically reopen it on inbound
  ...
  if (createError) {
    // Lost a race: a concurrent inbound delivery created the conversation
    // and the unique index rejected the duplicate. Re-resolve.
    if (isUniqueViolation(createError)) {
