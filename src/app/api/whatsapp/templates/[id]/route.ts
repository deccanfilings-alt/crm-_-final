...
export async function PATCH(
  ...
  try {
    ...
    // Resolve the caller's account_id and role so template + whatsapp_config
    ...
    if (profile?.account_role === 'viewer') {
      return NextResponse.json(
        { error: 'Viewers have read-only access and cannot edit templates.' },
        { status: 403 },
      )
    }
    ...
    // We need the existing row to read meta_template_id and status — fetch explicitly.
    ...
    if (!isDryRun()) {
      ...
      if (configError || !config || !config.waba_id) {
        return NextResponse.json(
          { error: 'WhatsApp not configured — cannot edit on Meta.' },
          { status: 400 },
        )
      }
      ...
      try {
        await editMessageTemplate({
          ...
          accessToken,
          category: payload.category.toUpperCase() as 'MARKETING' | 'UTILITY',
    ...
    const { data: row, error: updErr } = await adminDb
      .from('message_templates')
      .update({
...
export async function DELETE(
  ...
  try {
    ...
    if (profile?.account_role === 'viewer') {
      return NextResponse.json(
        { error: 'Viewers have read-only access and cannot delete templates.' },
        { status: 403 },
      )
    }
