...
async function upsertTemplateRow(
  ...
  // Use privileged client so Team Members (agents) can persist templates without RLS error
...
export async function POST(request: Request) {
  try {
    ...
    // Resolve the caller's account_id and role — whatsapp_config + the
    ...
    if (profile?.account_role === 'viewer') {
      return NextResponse.json(
        { error: 'Viewers have read-only access and cannot create templates.' },
        { status: 403 },
      )
    }
