...
import { extractTemplateVariables } from './template-validators';
...
function buildHeaderComponent(
  ...
  if (headerType === 'text') {
    ...
    const vars = extractTemplateVariables(template.header_content ?? '');
...
function buttonNeedsSendParam(
  ...
  switch (button.type) {
    case 'URL':
      return extractTemplateVariables(button.url).length > 0;
    case 'COPY_CODE':
