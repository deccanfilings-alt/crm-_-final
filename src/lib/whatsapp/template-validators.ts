...
/**
 * Extract all {{...}} placeholders from text, supporting both positional
 * `{{1}}` and named `{{name}}` Meta template parameters.
 */
export function extractTemplateVariables(text: string): TemplateVariable[] {
  ...
  for (const m of matches) {
    ...
    if (!isNaN(num) && num >= 1) {
      result.push({ raw, isNamed: false, index: num - 1 });
    } else {
      result.push({ raw, isNamed: true, name: raw, index: idx });
    }
    idx++;
  }
  return result;
